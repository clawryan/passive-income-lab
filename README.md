...
- **导入后可直接生成跟进待办**：线索区现可按当前筛选范围一键复制、手机原生分享“跟进待办”摘要，或导出结构化 JSON / Markdown 待办包；导入线索后状态栏也会直接提示当前可推进待办数，减少再手工整理到任务工具的摩擦。
...
- `npm run smoke:lead-todos`：载入演示线索并验证“当前筛选跟进待办”摘要与 JSON 导出链路
- `npm run validate`：串联执行以上五步
