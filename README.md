...
- **公开询价已自动保留来源归因**：当对外询价链接带 `src`、`ref` 或 `utm_source / utm_medium / utm_campaign / utm_content / utm_term` 时，页面提交询价会自动把这些归因字段写入线索与 `/api/lead-capture` 快照；返回摘要里也会额外给出 `sourceTagCounts / utmSourceCounts / utmCampaignCounts` 与 Top 项，方便后续判断哪种渠道/活动真的带来询价。
- **渠道追踪询价链接包**：公开询价区现支持批量输入 `feishu-dm / wechat-group / xhs-post` 等来源标签，并一键复制或导出对应的追踪询价链接包，便于直接分发到多个渠道，再回看来源日报判断哪个入口更有效。
- **渠道分发文案包**：在批量追踪链接之外，现可按来源标签一键生成对应的分发话术包（复制文本或导出 Markdown），把“生成追踪链接”直接推进到“今天就能外发实验”。
- **渠道实验清单**：公开询价区现可把同一批来源标签整理成“今日先发哪些渠道 / 每条追踪链接对应什么成功信号 / 24h 后看什么 / 无反馈时先改什么”的实验清单，并导出 JSON，方便手机端执行、协作者接力或后续自动化读取。
- **7 天渠道分发节奏 / 每日复盘模板**：公开询价区现可继续把同一批来源标签展开成 7 天分发节奏，自动给出 Day 1~Day 7 的目标、动作、复盘问题，以及一份每日复盘模板；适合把“今天发一下”推进到“连续一周都知道明天做什么”。
- **7 天节奏执行状态可直接标记 / 导出**：公开询价区现已补上节奏执行看板，可按当前产品把 Day 1~Day 7 逐天切换为“待执行 / 已执行 / 已复盘”，并一键复制执行摘要或导出执行 JSON；适合在手机和电脑之间接力，不必靠脑子记“这一周已经做到哪天”。
- **剩余节奏提醒可重导入日历**：在节奏执行看板旁现可一键导出“剩余节奏 ICS”，会自动跳过已复盘天数，只保留当前尚未完成的 Day 项，适合中途换设备、补录日历或重新规划接下来几天的提醒。
...
- **成交素材外发记录可筛选 / 导出**：每次从页面把“成交案例 / 已报价催单 / 复购转介绍”推到 Webhook 时，浏览器会额外留存最近 12 次外发摘要（素材类型、条数、主产品、主来源、时间）；现在还可按素材类型筛选，并一键复制摘要、导出 Markdown / JSON，方便在手机或电脑端快速确认今天已经推过哪些成交素材，避免重复骚扰同一批线索。
- **成交素材外发记录可推送 / 拉取远程快照**：在线索区现可把最近成交素材外发记录推送到 `/api/lead-asset-history`，或从远程快照拉回并合并到本地；默认会复用已配置的 Lead Capture 授权头，并优先把 `/api/lead-capture` 自动换成同源的 `/api/lead-asset-history`，更适合手机与电脑之间接力复盘。
...
- **跟进待办支持按产品/来源/阶段过滤**：`/api/lead-followup-todos` 与 `/api/cron/lead-followup-todos` 现支持通过 query/body 传入 `productSlug`、`source`、`stage`、`cadenceLevel`、`priority`（支持逗号分隔），可直接生成某个产品线、某个渠道或某个催办节奏的专用日报，更适合挂到手机快捷指令、低代码自动化或分产品 cron。
...
- **成交案例摘要 / Markdown / JSON 导出**：线索区现可把所有“已成交”线索聚合成可复制的案例摘要，或导出成 Markdown 案例集 / JSON 结构化案例包；同时支持手机原生分享成交案例，方便继续发朋友圈、私聊背书、补落地页社会证明，或交给自动化 / 协作者继续处理。
- **已报价催单 / 复购转介绍素材包**：线索区现可把“已报价”线索整理成一键复制、手机原生分享、Markdown / JSON 导出的催单摘要，把“已成交”线索整理成可复制、可分享、Markdown / JSON 导出的复购 / 转介绍跟进包，方便直接在飞书 / 微信私聊里推进成交和二次变现，也便于自动化读取。
